...
public class Login extends JFrame {
    ...
    public Login() {
        ...
        loginButton.addActionListener(e -> {
            ...
            // Check if it's a teacher ID (starts with "T" followed by 6 digits)
            if (enteredID.matches("T\\d{6}")) {
                // Initialize teacher data manager
                TeacherDataManager.initialize();
                
                // Authenticate teacher
                if (TeacherDataManager.authenticateTeacher(enteredID, enteredPassword)) {
                    dispose(); // close login
                    new ISLUTeacherPortal(enteredID).setVisible(true); // open Teacher Portal
                } else {
                    // If teacher authentication failed
                ...
                // Authenticate against Database.txt for students
                if (authenticateUser(enteredID, enteredPassword)) {
                    // If authentication successful → open student portal
                    dispose(); // close login
                    new ISLUStudentPortal(enteredID).setVisible(true); // open Student Portal
                } else {

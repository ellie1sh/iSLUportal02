...
public class ISLUStudentPortal extends JFrame {
    ...
    public ISLUStudentPortal(String studentID) {
        ...
        setupLayout(convertSubMenuItemsToStrings(PortalUtils.createHomeSublist()));
        loadAnnouncements();
    ...
    /**
     * Helper method to convert SubMenuItem list to String list for backward compatibility
     */
    private MySinglyLinkedList<String> convertSubMenuItemsToStrings(MySinglyLinkedList<SubMenuItem> subMenuItems) {
    ...
    private JPanel createGradesPanel(MySinglyLinkedList<String> subItems) {
        ...
        loadStudentGradesData(tableModel);
    ...
    private void loadStudentStatus() {
        ...
        // Load announcements from SharedDataManager for students
        MySinglyLinkedList<Announcement> studentAnnouncements = SharedDataManager.getStudentAnnouncements();
        
        if (studentAnnouncements.size() == 0) {
            status.append("No announcements from instructors.\n\n");
        } else {
            for (int i = 0; i < studentAnnouncements.size(); i++) {
                Announcement announcement = studentAnnouncements.get(i);
                status.append("[").append(announcement.getPriority()).append("] ");
                status.append(announcement.getTitle()).append("\n");
                status.append("Posted by: ").append(announcement.getAuthor()).append(" - ");
                status.append(announcement.getDateCreated()).append("\n\n");
                status.append(announcement.getContent()).append("\n");
                status.append("─────────────────────────────────────────────────────\n\n");
            }
        }
...
// Clear current content
        ...
        switch (item.getName()) {
            case "🏠 Home":
                ...
                contentPanel.setBorder(new EmptyBorder(20, 20, 20, 20));
                contentPanel.setBackground(Color.WHITE);
                setupLayout(convertSubMenuItemsToStrings(item.getSubItems()));
                break;
            case "📚 Journal/Periodical":
                contentPanel.add(createJournalPeriodicalPanel(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            case "📅 Schedule":
                contentPanel.add(createClassChecklistPanel());
                break;
            case "📌 Attendance":
                contentPanel.add(showAttendanceContent(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            case "📊 Grades":
                contentPanel.add(createGradesPanel(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            ...
            case "✅ Curriculum Checklist":
                contentPanel.add(createCurriculumChecklistPanel(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            case "ℹ️ Downloadable/ About iSLU":
                contentPanel.add(createAboutISLUPanel(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            case "🏥 Medical Record":
                contentPanel.add(createMedicalRecordPanel(convertSubMenuItemsToStrings(item.getSubItems())));
                break;
            default:
...
// Generic Content
    ...
    private JPanel createStatementOfAccountsPanel(MySinglyLinkedList<String> subItems) {
        // Use the new StatementOfAccountsPanel class
        StatementOfAccountsPanel soaPanel = new StatementOfAccountsPanel(studentID, studentName, studentProgram);
        return soaPanel;
    }
    ...
    private void loadStudentGradesData(DefaultTableModel model) {
        // Load student's grade data from SharedDataManager
        MySinglyLinkedList<GradeRecord> studentGrades = SharedDataManager.getStudentGrades(studentID);
        
        if (studentGrades.size() == 0) {
            // If no grades found, show actual subjects with "Not Yet Submitted"
            model.addRow(new Object[]{"7024", "NSTP-CWTS 1", "3", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9454", "GSTS", "3", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9455", "GENVI", "3", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9456", "CFE 103", "3", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9457", "IT 211", "3", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9458A", "IT 212", "2", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9458B", "IT 212L", "1", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9459A", "IT 213", "2", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9459B", "IT 213L", "1", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
            model.addRow(new Object[]{"9547", "FIT OA", "2", "", "", "", "Not Yet Submitted", "30% 30% 40%"});
        } else {
            // Load real grade data
            for (int i = 0; i < studentGrades.size(); i++) {
                GradeRecord record = studentGrades.get(i);
                double average = (record.getPrelimGrade() + record.getMidtermGrade() + record.getFinalGrade()) / 3.0;
                model.addRow(new Object[]{
                    record.getSubjectCode(),
                    record.getSubjectName(),
                    "3", // Default units
                    record.getPrelimGrade() > 0 ? record.getPrelimGrade() : "",
                    record.getMidtermGrade() > 0 ? record.getMidtermGrade() : "",
                    record.getFinalGrade() > 0 ? Math.round(average * 10.0) / 10.0 : "",
                    record.getFinalGrade() > 0 ? Math.round(average * 10.0) / 10.0 : "Not Yet Submitted",
                    "30% 30% 40%"
                });
            }
        }
    ...
    private void loadStudentAttendanceData(DefaultTableModel model) {
        // Load student's attendance data from SharedDataManager
        MySinglyLinkedList<AttendanceRecord> studentAttendance = SharedDataManager.getStudentAttendance(studentID);
        
        if (studentAttendance.size() == 0) {
            // If no attendance data found, show all subjects with "Not Recorded" status
            String[] subjects = {"7024", "9454", "9455", "9456", "9457", "9458A", "9458B", "9459A", "9459B", "9547"};
            String[] subjectNames = {"NSTP-CWTS 1", "GSTS", "GENVI", "CFE 103", "IT 211", "IT 212", "IT 212L", "IT 213", "IT 213L", "FIT OA"};
            String[] sampleDates = {"2025-01-15", "2025-01-17", "2025-01-20", "2025-01-22", "2025-01-24"};
            
            for (int i = 0; i < subjects.length; i++) {
                for (String date : sampleDates) {
                    model.addRow(new Object[]{
                        subjects[i] + " - " + subjectNames[i],
                        date,
                        "Not Recorded",
                        "Attendance not yet recorded by teacher"
                    });
                }
            }
        } else {
            ...
            for (int i = 0; i < studentAttendance.size(); i++) {
                AttendanceRecord record = studentAttendance.get(i);
                model.addRow(new Object[]{
                    record.getSubjectCode() + " - " + TeacherDataManager.getSubjectName(record.getSubjectCode()),
    ...
    private void loadStudentClassChecklistData(DefaultTableModel model) {
        // Load student's class checklist data from SharedDataManager
        MySinglyLinkedList<ClassChecklistItem> studentChecklist = SharedDataManager.getStudentClassChecklist(studentID);
        
        if (studentChecklist.size() == 0) {
            // If no checklist data found, show all subjects with "Pending" status
            String[] subjects = {"7024", "9454", "9455", "9456", "9457", "9458A", "9458B", "9459A", "9459B", "9547"};
            String[] subjectNames = {"NSTP-CWTS 1", "GSTS", "GENVI", "CFE 103", "IT 211", "IT 212", "IT 212L", "IT 213", "IT 213L", "FIT OA"};
            ...
            // Load real checklist data
            for (int i = 0; i < studentChecklist.size(); i++) {
                ClassChecklistItem item = studentChecklist.get(i);
                model.addRow(new Object[]{
                    item.getSubjectCode() + " - " + TeacherDataManager.getSubjectName(item.getSubjectCode()),
                    item.getRequirement(),
                    item.getStatus(),
                    item.getRemarks()
                });
